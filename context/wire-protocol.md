...
- Single source of truth: `crates/afterray-protocol`. `Request` enum (src/lib.rs:22, ~50 variants, `#[serde(tag = "type", rename_all = "snake_case")]`); `Response` envelope `{protocol_version, ok, data?, error?}` (lib.rs:275); `PROTOCOL_VERSION = 10` (lib.rs:18).
