...
- Single source of truth: `crates/afterray-protocol`. `Request` is tagged snake_case; responses use `{protocol_version, ok, data?, error?}`; `PROTOCOL_VERSION = 12`.
...
- Agent screenshot citations add no wire shape: a standalone `![label](afterray://moment/ID)` is parsed by the Swift chat surface, which fetches bytes through the existing `ReadThumbnail` request. General Markdown image URLs are never fetched; deleted or invalid moments degrade to a clickable text citation.
- Swift mirror: `swift/AfterRayRecall/Sources/DaemonClient.swift` — `UnixSocketDaemonClient`, a hand-declared `WireRequest` with snake_case CodingKeys, and `protocolVersion = 12` enforced on every response. **Bump Rust and Swift together — there is no negotiation; a mismatch fails every request with `protocolMismatch`.**
