...
- `protocolVersion` must stay in lockstep with `PROTOCOL_VERSION: u32 = 11` (`crates/afterray-protocol/src/lib.rs:18`); bump both on any wire change or every request fails with `protocolMismatch`.
